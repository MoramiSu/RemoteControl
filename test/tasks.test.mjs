...
async function choose(service,s,id,messageId='use',now=1001){
 ...
 const worker=new ProjectWorker(s,client);service.accept(event('/rc tasks','list-'+messageId),now);await worker.tick(now);const list=JSON.parse(s.db.prepare("SELECT items FROM project_lists WHERE kind='tasks'").get().items);const result=service.accept(event('/rc task '+(list.findIndex(x=>x.id===id)+1),messageId),now);await worker.tick(now);return result;
}
test('switch freezes queued targets and duplicate use cannot replay after service restart',async t=>{
 const {s,service,opts}=setup(t);service.accept(event('A original','a'),1001);await choose(service,s,B);service.accept(event('B original','b'),1002);await choose(service,s,A,'back');const restored=new ReceiveService(s,opts);assert.equal(restored.accept(event('/rc task 2','use'),1003).duplicate,true);assert.equal(restored.binding.target_id,A);assert.deepEqual(s.db.prepare("SELECT original_text,target_id FROM inbox WHERE delivery='queued' ORDER BY id").all().map(r=>[r.original_text,r.target_id]),[['A original',A],['B original',B]]);
});
test('unknown sender and missing list never change binding',async t=>{
 const {s,service}=setup(t);assert.equal(service.accept(event('/rc task 2','intruder','other'),1002).accepted,false);service.accept(event('/rc task 999','invalid'),1002);const worker=new ProjectWorker(s,{call:async()=>{throw Error('should not call')},close(){}});await worker.tick(1002);assert.equal(s.db.prepare('SELECT target_id FROM bindings').get().target_id,A);assert.match(s.pendingReplies().at(-1).body,/列表已过期/);
});
