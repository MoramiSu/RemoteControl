...
test('project selection filters tasks, blocks old-target text and preserves prior queue and pause',async t=>{
 const f=setup(t);f.send('old','old');f.send('/rc projects');await f.worker.tick(1001);f.send('/rc project 1');const early=f.send('early');assert.equal(f.store.get(early.id).delivery,'cancelled');await f.worker.tick(1002);assert.equal(selectedProject(f.store,'t','c').needsTask,true);f.send('/rc tasks');await f.worker.tick(1003);const list=JSON.parse(f.store.db.prepare("SELECT items FROM project_lists WHERE kind='tasks'").get().items);assert.equal(list.length,1);f.store.pause('t','c',true);f.send('/rc task 1');await f.worker.tick(1004);assert.equal(f.store.db.prepare('SELECT target_id FROM bindings').get().target_id,task.id);assert.equal(f.store.db.prepare('SELECT paused FROM bindings').get().paused,1);assert.equal(f.store.get(1).target_id,TEST_TARGET);assert.equal(allowedTasks(f.store).get(task.id),task.title);const next=f.send('next');assert.equal(f.store.get(next.id).target_id,task.id);
});
...
test('global default excludes project tasks; exiting invalidates list and blocks old project delivery',async t=>{
 ...
 f.send('/rc projects');await f.worker.tick(1002);f.send('/rc project 1');await f.worker.tick(1003);f.send('/rc tasks');await f.worker.tick(1004);f.send('/rc task 1');await f.worker.tick(1005);assert.equal(f.store.db.prepare('SELECT target_id FROM bindings').get().target_id,task.id);const old=f.send('project work');
 ...
 f.send('/rc tasks');await f.worker.tick(1007);f.send('/rc task 1');await f.worker.tick(1008);assert.equal(globalSelectionPending(f.store,'t','c'),false);const next=f.send('global work');assert.equal(f.store.get(next.id).target_id,'loose');
});
test('tasks respects current scope and stale cross-project snapshot cannot switch',async t=>{
 ...
 f.store.db.prepare("UPDATE project_lists SET items=? WHERE kind='tasks'").run(JSON.stringify([{id:task.id,title:task.title,projectId:'p',scope:'recent'}]));f.send('/rc task 1');await f.worker.tick(1002);assert.equal(f.store.db.prepare('SELECT target_id FROM bindings').get().target_id,TEST_TARGET);
